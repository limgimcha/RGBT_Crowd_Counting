...
class SASNet(nn.Module):
    ...
    def forward(self, RGBT):
        ...
        #print("x5_density",x5_density.shape)
        ...
        #print("x1_density",x1_density.shape)
        ...
        #print("x1_confi",x1_confi.shape)
        # upsample the density prediction to be the same with the input size
        x5_density = F.interpolate(x5_density,size=(size_x,size_y),mode='bilinear',align_corners=True)
        x4_density = F.interpolate(x4_density,size=(size_x,size_y),mode='bilinear',align_corners=True)
        x3_density = F.interpolate(x3_density,size=(size_x,size_y),mode='bilinear',align_corners=True)
        x2_density = F.interpolate(x2_density,size=(size_x,size_y),mode='bilinear',align_corners=True)
        x1_density = F.interpolate(x1_density,size=(size_x,size_y),mode='bilinear',align_corners=True)
        ...
        #print("x5_confi_upsample",x5_confi_upsample.shape)
        ...
        #print("x1_confi_upsample",x1_confi_upsample.shape)
        # =============================================================================================================
        # soft √
        confidence_map = torch.cat([x5_confi, x4_confi,
                                    x3_confi, x2_confi, x1_confi], 1)
