...
class FusionModel(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        #self.conv2 = nn.Conv2d(256, c3, kernel_size=1, stride=1, padding=0, bias=False)
        #self.conv3 = nn.Conv2d(c3, c2, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        # Top layer
        self.toplayer = nn.Conv2d(2048, c1, kernel_size=1, stride=1, padding=0)  # Reduce channels

        # Smooth layers
        self.smooth1 = nn.Conv2d(c1, c1, kernel_size=3, stride=1, padding=1)
        self.smooth2 = nn.Conv2d(c1, c1, kernel_size=3, stride=1, padding=1)
        self.smooth3 = nn.Conv2d(c1, c1, kernel_size=3, stride=1, padding=1)

        # Lateral layers
        self.latlayer1 = nn.Conv2d(1024, c1, kernel_size=1, stride=1, padding=0)
        self.latlayer2 = nn.Conv2d( 512, c1, kernel_size=1, stride=1, padding=0)
        self.latlayer3 = nn.Conv2d( 256, c1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, RGBT):
        ...
        rp2 = F.upsample_bilinear(rp2, scale_factor=2)
        tp2 = F.upsample_bilinear(tp2, scale_factor=2)
        #print("tp2: ",tp2.shape)
