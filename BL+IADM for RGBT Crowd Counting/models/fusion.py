...
class FusionModel(nn.Module):
    def __init__(self, ratio=0.6):
        ...
        self.resBlock1 = ResBlock(3, in_channels, stride=1)
        self.block2 = Block([c2, c2, 'M'], in_channels=c1)
        self.resBlock2 = ResBlock(c1, in_channels, stride=1)
        self.block3 = Block([c3, c3, c3, c3, 'M'], in_channels=c2)
        self.resBlock3 = ResBlock(c2, in_channels, stride=1)
        self.block4 = Block([c4, c4, c4, c4, 'M'], in_channels=c3)
        self.resBlock4 = ResBlock(c3, in_channels, stride=1)
        self.block5 = Block([c4, c4, c4, c4], in_channels=c4)
        self.resBlock5 = ResBlock(c4, in_channels, stride=1)
    ...
    def forward(self, RGBT):
        ...
        RGB, T, shared = self.resBlock1(RGB, T, shared)
        RGB, T, shared = self.block2(RGB, T, shared)
        RGB, T, shared = self.resBlock2(RGB, T, shared)
        RGB, T, shared = self.block3(RGB, T, shared)
        RGB, T, shared = self.resBlock3(RGB, T, shared)
        RGB, T, shared = self.block4(RGB, T, shared)
        RGB, T, shared = self.resBlock4(RGB, T, shared)
        _, _, shared = self.block5(RGB, T, shared)
        _, _, shared = self.resBlock1(RGB, T, shared)
        x = shared
...
class ResBlock(nn.Module):
    def __init__(self, num_filters, in_channels=None, stride=1):
        super(ResBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, num_filters, kernel_size=3, stride=stride, padding=1)
    ...
    def forward(self, RGB, T, shared):
        ...
        new_RGB = self.conv1(RGB)
        new_T = self.conv1(T)
        new_shared = self.conv1(shared)

        new_RGB = self.bn1(new_RGB)
        new_T = self.bn1(new_T)
        new_shared = self.bn1(new_shared)

        new_RGB = self.relu1(new_RGB)
        new_T = self.relu1(new_T)
        new_shared = self.relu1(new_shared)

        new_RGB = self.conv2(new_RGB)
        new_T = self.conv2(new_T)
        new_shared = self.conv2(new_shared)

        new_RGB = self.bn2(new_RGB)
        new_T = self.bn2(new_T)
        new_shared = self.bn2(new_shared)

        new_RGB = self.dropout(new_RGB)
        new_T = self.dropout(new_T)
        new_shared = self.dropout(new_shared)


        #residual = self.projection(x)
        new_RGB += residual
        new_T += residual_T
        new_shared += residual_s

        new_RGB = self.relu2(new_RGB)
        new_T = self.relu2(new_T)
        new_shared = self.relu2(new_shared)
